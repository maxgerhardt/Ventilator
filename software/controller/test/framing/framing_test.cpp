...
TEST(FramingTests, ControllerStatusCoding) {
  ...
  DmaStream<200> dma_stream{&mock_uart_dma};
  ...
  EXPECT_EQ(mock_uart_dma.tx_length_, encoded_length);
  ControllerStatus decoded = ControllerStatus_init_zero;
  DecodeResult r = DecodeControllerStatusFrame(mock_uart_dma.tx_buffer_, encoded_length, &decoded);
...
TEST(FramingTests, GuiStatusCoding) {
  ...
  DecodeResult r = DecodeGuiStatusFrame(mock_uart_dma.tx_buffer_, encoded_length, &decoded);
  ASSERT_EQ(r, DecodeResult());
