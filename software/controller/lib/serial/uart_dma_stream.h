...
class DmaStream : public OutputStream, public TxListener {
 public:
  explicit DmaStream(UartDma *uart_dma) : uart_dma_(uart_dma){};
  ...
  size_t bytes_available_for_write() override {
    // TODO thread safety
    if (uart_dma_->tx_in_progress()) {
  ...
  void on_tx_complete() override {
    if (schedule_tx_) {
      ...
      [[maybe_unused]] bool started = uart_dma_->start_tx(current_buffer_, index_, this);
      swap_buffers();
      schedule_tx_ = false;
    }
  }
 ...
 private:
  ...
  ResponseFlags transmit() {
    if (is_buf_full() && uart_dma_->tx_in_progress()) {
      schedule_tx_ = true;
